(function () {
  const DEFAULT_INDEX_WEIGHTS = {
    performance: {
      goalContributions: 0.45,
      goalContributionsPerAppearance: 0.25,
      attendance: 0.3
    },
    contribution: {
      attendance: 0.45,
  ...
  function buildPlayerIndexes(players, settings, attendanceDates, options = {}) {
    ...
    const maxGoalContributions = Math.max(
      ...safePlayers.map((player) => {
        const stats = player?.stats || {};
        return (Number(stats.goals) || 0) + (Number(stats.assists) || 0) * 0.7;
      }),
      0
    );
    const baseRows = safePlayers.map((player) => {
      ...
      const goalContributions = goals + assists * 0.7;
      const goalContributionsPerAppearance =
        attendance.present > 0 ? goalContributions / attendance.present : 0;
      return { player, goals, assists, attendance, goalContributions, goalContributionsPerAppearance };
    });
    const maxGoalContributionsPerAppearance = Math.max(
      ...baseRows.map((row) => row.goalContributionsPerAppearance),
      0
    );
    ...
    const rows = baseRows.map((row) => {
      ...
      const goalContributionsScore =
        maxGoalContributions > 0 ? (row.goalContributions / maxGoalContributions) * 100 : 0;
      const goalContributionsPerAppearanceScore =
        maxGoalContributionsPerAppearance > 0
          ? (row.goalContributionsPerAppearance / maxGoalContributionsPerAppearance) * 100
          : 0;
      const performanceRaw = weightedAverage(
        {
          goalContributions: goalContributionsScore,
          goalContributionsPerAppearance: goalContributionsPerAppearanceScore,
      ...
      return {
        ...
        metrics: {
          ...
          goalContributionsPerAppearance: row.goalContributionsPerAppearance,
