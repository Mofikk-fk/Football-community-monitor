(function () {
  ...
  async function getSession() {
    return { user: { id: "demo-admin", email: "kemi@achieversfc.demo", user_metadata: { full_name: "Kemi Adewale" } } };
  }
  ...
  function injectTopbarActions(session, me) {
    ...
    userMenu.innerHTML = `
      <button type="button" id="user-menu-trigger" class="user-menu-trigger" aria-expanded="false" aria-haspopup="menu">
        ...
        <span class="demo-mode-indicator">Demo Mode &bull; Sample Data</span>
        <i class="fa-solid fa-chevron-down user-chevron" aria-hidden="true"></i>
      </button>
      <div id="user-menu-dropdown" class="user-menu-dropdown hidden" role="menu"><span class="dropdown-item demo-mode-label">Demo Mode &bull; Sample Data</span></div>
    `;
  ...
  window.apiFetch = async function apiFetch(path, options = {}) {
    pendingRequests += 1;
    try {
      return await window.demoApi.request(path, options);
    } finally {
