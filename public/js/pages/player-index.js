(function () {
  ...
  const DEMO_MOVEMENTS = {
    overall: { "player-2": "up", "player-4": "up", "player-9": "up", "player-16": "up", "player-23": "up", "player-6": "down", "player-12": "down", "player-19": "down", "player-28": "down" },
    performance: { "player-1": "down", "player-5": "up", "player-8": "up", "player-14": "down", "player-18": "up", "player-22": "down", "player-25": "up", "player-30": "down" },
    contribution: { "player-3": "up", "player-7": "down", "player-11": "up", "player-15": "up", "player-20": "down", "player-24": "up", "player-26": "down", "player-29": "up" },
    attendance: { "player-2": "down", "player-6": "up", "player-10": "up", "player-13": "down", "player-17": "up", "player-21": "down", "player-27": "up", "player-30": "down" }
  };
  ...
  function getMovement(rankingKey, player) {
    const demoMovement = DEMO_MOVEMENTS[rankingKey]?.[player.id];
    if (demoMovement === "up") return { className: "up", symbol: "▲" };
    if (demoMovement === "down") return { className: "down", symbol: "▼" };
    if (window.demoApi) return { className: "same", symbol: "▬" };
      ...
      <tr class="player-index-detail-row hidden" data-detail-for="${escapeHtml(player.id)}">
        <td colspan="${colspan}">
          <div class="player-index-detail-grid">
            ...
            <div class="detail-item"><span>Assists</span><strong>${metrics.assists}</strong></div>
            <div class="detail-item"><span>Weighted G+A / Appearance</span><strong>${formatDecimal(metrics.goalContributionsPerAppearance)}</strong></div>
  ...
  function renderSummary() {
    ...
    const bestGoalContributionsPerAppearance = [...state.rows].sort((a, b) => {
      if (b.metrics.goalContributionsPerAppearance !== a.metrics.goalContributionsPerAppearance) {
        return b.metrics.goalContributionsPerAppearance - a.metrics.goalContributionsPerAppearance;
      }
      return String(a.name || "").localeCompare(String(b.name || ""));
    })[0];
    ...
    summary.goalsPerAppearance.textContent = bestGoalContributionsPerAppearance
      ? formatDecimal(bestGoalContributionsPerAppearance.metrics.goalContributionsPerAppearance)
      : "0.00";
  ...
  function renderTables() {
    ...
    renderRanking(bodies.performance, rankings.performance, 7, (player, index) => `
      <tr>
        <td data-label="Rank">${rankCell("performance", player, index)}</td>
        <td data-label="Player">${escapeHtml(player.name || "Player")}</td>
        <td data-label="Goals">${player.metrics.goals}</td>
        <td data-label="Assists">${player.metrics.assists}</td>
        <td data-label="Weighted G+A per Appearance">${formatDecimal(player.metrics.goalContributionsPerAppearance)}</td>
